...
plosh = lambda a, b: 3.14 * a * b
list = []
max = plosh(orbits[0][0], orbits[0][1])
maxindex = 0
for i in range(len(orbits)):
    for j in range(1):
        if orbits[i][j] != orbits[i][j + 1]:
            list.append(plosh(orbits[i][j], orbits[i][j + 1]))
    if max < list[i]:
        max = float(list[i])
        maxindex = i

print(max(list), orbits[maxindex])
